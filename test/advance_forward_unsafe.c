...
static void test_zero(void) {
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe(mixed, mixed_len, 0, &advanced) == 0,
        "zero: offset 0");
...
static void test_one(void) {
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe(mixed, mixed_len, 1, &advanced) == 1,
        "one: offset 1");
...
static void test_two(void) {
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe(mixed, mixed_len, 2, &advanced) == 3,
        "two: offset 3");
...
static void test_three(void) {
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe(mixed, mixed_len, 3, &advanced) == 6,
        "three: offset 6");
...
static void test_exact(void) {
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe(mixed, mixed_len, 4, &advanced) == 10,
        "exact: offset 10");
...
static void test_exceed(void) {
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe(mixed, mixed_len, 5, &advanced) == 10,
        "exceed: offset 10");
  CHECK(advanced == 4, "exceed: advanced 4");

  CHECK(utf8_advance_forward_unsafe(mixed, mixed_len, 100, &advanced) == 10,
        "exceed 100: offset 10");
  CHECK(advanced == 4, "exceed 100: advanced 4");
}

static void test_empty(void) {
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe("", 0, 0, &advanced) == 0,
        "empty d=0: offset 0");
  CHECK(advanced == 0, "empty d=0: advanced 0");

  CHECK(utf8_advance_forward_unsafe("", 0, 1, &advanced) == 0,
        "empty d=1: offset 0");
...
static void test_ascii(void) {
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe("ABCDEFGH", 8, 3, &advanced) == 3,
        "ascii: d=3 offset 3");
  CHECK(advanced == 3, "ascii: d=3 advanced 3");

  CHECK(utf8_advance_forward_unsafe("ABCDEFGH", 8, 8, &advanced) == 8,
        "ascii: d=8 offset 8");
  CHECK(advanced == 8, "ascii: d=8 advanced 8");

  CHECK(utf8_advance_forward_unsafe("ABCDEFGHIJKLMNOP", 16, 12, &advanced) == 12,
        "ascii: d=12 offset 12");
...
static void test_2byte(void) {
  ...
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe(src, 8, 2, &advanced) == 4,
        "2byte: d=2 offset 4");
...
static void test_3byte(void) {
  ...
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe(src, 9, 2, &advanced) == 6,
        "3byte: d=2 offset 6");
...
static void test_4byte(void) {
  ...
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe(src, 8, 1, &advanced) == 4,
        "4byte: d=1 offset 4");
  CHECK(advanced == 1, "4byte: d=1 advanced 1");
}

static void test_null_advanced(void) {
  CHECK(utf8_advance_forward_unsafe(mixed, mixed_len, 2, NULL) == 3,
...
static void test_large_ascii(void) {
  ...
  size_t advanced;
  CHECK(utf8_advance_forward_unsafe(buf, sizeof buf, 1000, &advanced) == 1000,
        "large ascii: d=1000 offset 1000");
...
static void test_agrees_with_safe(void) {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    size_t r_safe   = utf8_advance_forward(cases[i].s, cases[i].len,
                                           cases[i].distance, &adv_safe);
    size_t r_unsafe = utf8_advance_forward_unsafe(cases[i].s, cases[i].len,
