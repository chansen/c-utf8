...
stream_check(const char* desc,
  ...
  TestCount++;
  if (got.carried != exp_carried) {
    printf("utf8_valid_stream_check [%s] carried == %zu, expected %zu at line %u\n",
           desc, got.carried, exp_carried, line);
    TestFailed++;
  }
}

#define STREAM(desc, st, src, len, eof, exp_status, exp_consumed, exp_pending, exp_advance, exp_carried) do { \
  utf8_valid_stream_result_t _r = utf8_valid_stream_check(st, src, len, eof); \
  stream_check(desc, exp_status, exp_consumed, exp_pending, exp_advance, exp_carried, _r, __LINE__); \
} while (0)
...
test_streaming(void) {
  ...
  STREAM("split then more 1", &st, "abc\xC3", 4, false,
         UTF8_VALID_STREAM_PARTIAL, 3, 1, 0, 0);
  ...
  STREAM("carried lead then ASCII", &st, "A", 1, false,
         UTF8_VALID_STREAM_ILLFORMED, 0, 0, 0, 1);
  ...
  STREAM("carried lead + cont then ASCII", &st, "\x82""A", 2, false,
         UTF8_VALID_STREAM_ILLFORMED, 0, 0, 1, 1);
